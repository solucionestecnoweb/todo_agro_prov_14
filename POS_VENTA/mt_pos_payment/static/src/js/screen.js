        ...
        class ReceiptScreen extends AbstractReceiptScreen {
            ...
            async ImprimirFiscal() {
                ...
                for (var i = 0; i < order.orderlines.models.length; i += 1) {
                    ...
                    lineas.push({
                        'product': order.orderlines.models[i].product.display_name.replace("&","**").replace("'","**"),
                ...
                window.open("http://localhost:8080/fiscal_13/cargar.php?cid=" + order.cid +"&numero_recibo=" + order.name + "&cliente=" + order.changed.client.name + "&telefono=" + order.changed.client.phone + "&direccion=" + order.changed.client.address + "&rif_cedula=" + order.changed.client.vat + "&lineas=" + enviar_lineas+"&order_id="+666, "width=300,height=500,scrollbars=YES")
